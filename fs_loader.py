...
class DataDescriptor(object):
    ...
    def __eq__(self, other):
        return (self.element, self.forecast_hour) == (other.element, other.forecast_hour)

    def __str__(self):
        return 'DataDescriptor(element: %s, forecast hour: %d)' % (self. element, self.forecast_hour)
...
def get_file_path(file_identifier):
    ...
    return 'data/mgforsys_europe/%s/fh%d_D11_B3_T2_M5.csv' % (element, forecast_hour)
...
def load_dataset_for_forecast_hour(forecast_hour, request):
    ...
    logging.debug("Data for forecast hour %d: loading %d file(s).." % (forecast_hour, len(files_to_load)))
    dataset_map = load_multiple_files(files_to_load)
    logging.debug("Data for forecast hour %d: successfully loaded %d files." % (forecast_hour, len(dataset_map)))
    ...
    for data_id in dataset_map:
        ...
        if data.empty:
            ...
            assert (data.dtypes[before_dtypes.index] == before_dtypes).all(), "Data types changed after merge."
...
def construct_dataset_column_names(request):
    ...
    dataset_column_names = ['station_id', 'latitude', 'longitude', 'elevation', 'forecast_hour', 'valid_date']
...
def load_dataset(request):
    ...
    for forecast_hour in request['forecast_hours']:
        ...
        # TODO TdR 13/07/16: filter out stations located outside of request
    ...
    forecast_data = forecast_data[column_names]
